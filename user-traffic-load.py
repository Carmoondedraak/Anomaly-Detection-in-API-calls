...
class UserWithOrders(User):
  ...
  def on_start(self):
    ...
    time1 = time.strftime("%a, %d %b %Y %H:%M:%S GMT", time.gmtime())
    res, data1 = client.register(username=self.user['username'], password=self.user['password'], email=self.user['email'],
                    firstName=self.user['firstName'], lastName=self.user['lastName'])
    
    time2 = time.strftime("%a, %d %b %Y %H:%M:%S GMT", time.gmtime())
    ...
    time3 = time.strftime("%a, %d %b %Y %H:%M:%S GMT", time.gmtime())
    res, data3 = client.add_credit_card(longnum=self.user['banknum'], expires=self.user['date'], ccv=self.user['ccv'])
    
    time4 = time.strftime("%a, %d %b %Y %H:%M:%S GMT", time.gmtime())
    
    self.coockie, data4= client.login(self.user['username'], self.user['password'])
    data_all = [(data1,time1),(data2, time2),(data3,time3),(data4,time4)]
    self.data += data_all

  def add_to_dataset(self):
      if len(self.data) > 0:
        dataset = Create_Dataset(self.data, self.coockie, self.ip_address)
        self.data = []
...
class Create_Dataset(object):
    ...
    def __init__(self, data, coockie, ip_address):
        ...
        if os.path.isfile('data/dataframe_new.pkl'):
            df_prev =  pd.read_pickle('data/dataframe_new.pkl')
            df = pd.concat([df,df_prev],ignore_index=True)
        df.to_pickle('data/dataframe_new.pkl')
