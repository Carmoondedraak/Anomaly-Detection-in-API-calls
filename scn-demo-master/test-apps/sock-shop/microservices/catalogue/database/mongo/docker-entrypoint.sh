...
if [[ "$originalArgOne" == mongo* ]] && [ "$(id -u)" = '0' ]; then
  if [ "$originalArgOne" = 'mongod' ]; then
    find /data/configdb /data/db \! -user mongo -exec chown mongo '{}' +
  fi

  # make sure we can write to stdout and stderr as "mongo"
  # (for our "initdb" code later; see "--logpath" below)
  chown --dereference mongo "/proc/$$/fd/1" "/proc/$$/fd/2" || :
  # ignore errors thanks to https://github.com/docker-library/mongo/issues/149

  exec su-exec mongo "$BASH_SOURCE" "$@"
fi
