...
API_ACCESS_TOKEN=$(curl https://api-m.sandbox.paypal.com/v1/oauth2/token -s -H "Accept: application/json" -H "Accept-Language: en_US" -u ${PAYPAL_ACCESS_CODE} -d "grant_type=client_credentials" | grep 'access_token' | awk  -F'"' '{print $8}')
